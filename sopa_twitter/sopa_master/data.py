...
#     """
        text = re.sub(r'https?:\/\/[^ ]+', '', text,flags=re.MULTILINE).strip()
...
#         docs = [line.rstrip().split() for line in input_file]
        tknzr = tokenize.TweetTokenizer(reduce_len=True,strip_handles=True)
        docs=[tknzr.tokenize(clean_text(line)) for line in input_file]
